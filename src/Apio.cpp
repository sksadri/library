...
void ApioClass::loadSettingsFromEeprom() {
  ...
  // Address 8180 - 2 bytes  - Network Identifier
  // Address 8182 - 2 bytes  - Network Address
  ...
  nwkSetSecurityKey((uint8_t *)buffer);
  memset(buffer, 0x00, 16);
  if (eeprom_read_word((uint16_t *)8182) != 0xFFFF ||
      eeprom_read_word((uint16_t *)8180) != 0xFFFF ||
      eeprom_read_byte((uint8_t *)8179) != 0xFF) {
    nwkSetRadio(eeprom_read_word((uint16_t *)8182), eeprom_read_word((uint16_t *)8180), eeprom_read_byte((uint8_t *)8179));
  }
  if (eeprom_read_byte((uint8_t *)8178) != 0xFF) {
    nwkSetPower(eeprom_read_byte((uint8_t *)8178));
  }
  if (eeprom_read_byte((uint8_t *)8126) != 0xFF) {
    nwkSetDataRate(eeprom_read_byte((uint8_t *)8126));
  }
}

void ApioClass::nwkSetRadio(const uint16_t theAddress, const uint16_t thePanId, const uint8_t theChannel) {
...
void ApioClass::nwkSetPower(const uint8_t theTxPower) {
...
void ApioClass::nwkSetDataRate(const uint8_t theRate) {
...
void ApioClass::nwkSetSecurityKey(const uint8_t *key) {
...
void ApioClass::nwkGetSecurityKey(char *key) {
  for (int i=0; i<16; i++) {
    key[i] = eeprom_read_byte((uint8_t *)8162+i);
  }
}

void ApioClass::nwkResetSecurityKey(void) {
  const uint8_t buf[16] = {0xFF, 0xFF, 0xFF, 0xFF,0xFF, 0xFF, 0xFF, 0xFF,0xFF, 0xFF, 0xFF, 0xFF,0xFF, 0xFF, 0xFF, 0xFF};
  nwkSetSecurityKey(buf);
}

void ApioClass::nwkListen(uint8_t endpoint, bool (*handler)(NWK_DataInd_t *ind)) {
  NWK_OpenEndpoint(endpoint, handler);
}

void ApioClass::nwkJoinGroup(uint16_t groupAddress) {
  if (!NWK_GroupIsMember(groupAddress)) {
    NWK_GroupAdd(groupAddress);
  }
}

void ApioClass::nwkLeaveGroup(uint16_t groupAddress) {
  if (NWK_GroupIsMember(groupAddress)) {
    NWK_GroupRemove(groupAddress);
  }
}

bool ApioClass::nwkIsInGroup(uint16_t groupAddress) {
